...
class Anken_Successrt(TenantBaseModel):
    """
案件成約確率のテーブル
[更新説明]
・1回/日の間隔にて更新を行う。
...
・全データを対象として、Delete + Insert を行う。
    ...
    def __str__(self):
        return 'ID:{0},CUSTOMER_NAME:{1},ANKEN_NAME:{2}' \
        .format(self.id, self.customer_name, self.anken_name)

# 管理者用画面での編集内容
class Anken_Successrt_Admin(admin.ModelAdmin):
    list_display = ('id', 'tenant_id', 'customer_name', 'anken_name', 'success_rate', 'updated_at')
    ordering = ('tenant_id', 'id')
    readonly_fields = ('id', 'created_at', 'updated_at')
    list_filter = ('tenant_id', )
    search_fields = ('customer_name', 'anken_name', )

# 管理者画面に表示
admin.site.register(Anken_Successrt, Anken_Successrt_Admin)
