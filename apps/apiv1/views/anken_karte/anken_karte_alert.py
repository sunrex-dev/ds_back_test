...
class AnkenKarteAlertSerial(serializers.ModelSerializer):
    class Meta:
        model=Anken_Advice
        fields = ('id', 'customer_id', 'anken_id', 'rulebook_comment', 'rulebook_rank', 'last_visit_dt')

class AnkenKarteAlertListView(generics.ListAPIView):
    permission_classes = (IsAuthenticated,)
    serializer_class = AnkenKarteAlertSerial
    ...
    def get_queryset(self):
        ...
        query = query.filter(rulebook_ruletype=2)
    ...
    @swagger_auto_schema(operation_summary="[案件カルテ]の[アラート]取得", manual_parameters=[
